...
using namespace std;
/*
原本计划用一个mem数组记录每一个位置与前一个位置相比是升还是降，但是遇到不升不降的情况的时候不太好处理（后序节点不知道前面怎么变化）
*/
//使用动态规划的办法
//如果当前是一个升位置，up在down位置基础上更新；
//如果当前是一个降位置，down在up位置基础上更新。
int wiggleMaxLength(vector<int>& nums) {
    ...
    int up=1;//记录一个上升的位置
    int down=1;//记录一个下降的位置
    for(int i=1;i<n;i++)
    {
        if(nums[i]>nums[i-1])//当前是一个上升的位置
        up=down+1;
        else if (nums[i]<nums[i-1])//当前是一个下降的位置
        {
            down=up+1;
        }
    }
