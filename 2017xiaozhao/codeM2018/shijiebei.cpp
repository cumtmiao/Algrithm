...
#include<bits/stdc++.h>
using namespace std;
vector<int> calculation(vector< vector<int> > chushi)
{
    vector<int> aftertaotai;
    for(int i=0;i<16;i++)
    {
        int temp=0;
        for(int j=0;j<16;j++)
        ...
        aftertaotai.push_back(temp*1.0/15);
    }
}
int main()
{
    vector< vetcor<int> > chushi;
    vector<int> res;
    for(int i=0;i<16;i++)
    {
        vector<int> tempv;
        ...
        chushi.push_back(tempv);
    }
    res=caculation(chushi);
}