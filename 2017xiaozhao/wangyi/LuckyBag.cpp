...
（ak必然大于等于max(a1,a2,...,ak-1)），继续向后选择更大的数，必然无法满足！因此，可以进行剪枝。
...
int res=0;
int n;
int numberofbags(int *data,int index,int sum,int multi)//dfs深度优先遍历--参数：数组指针，表示深度的index，表示的和，表示的积
{
    int count=0;
    for(int i=index;i<n;i++)
    {
        ...
        multi*=data[i];
        if(sum>multi)
        count+=1+numberofbags(data,i+1,sum,multi);
        else if(data[i]==1)//剪枝
        {
            count+=numberofbags(data,i+1,sum,multi);
        }
        else//剪枝
        break;
        sum-=data[i];
...
int main()
{
    ...
    sort(data,data+n);
    cout<<numberofbags(data,0,sum,multi);
    return 0;
