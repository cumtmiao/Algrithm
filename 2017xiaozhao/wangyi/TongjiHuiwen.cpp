...
#include<bits/stdc++.h>
using namespace std;
bool ishuiwen(string s)//判断一个字符串是不是回文串
{
    int length=s.size();
    int left=0;
    int right=length-1;
...
int numberofhuiwen(string s1,string s2)
{
    int length1=s1.size();
    int num=0;
    ...
    {
       string s3="";
       s3=s1.substr(0,i)+s2+s1.substr(i,length1-i);//将字符串S1和s2重构成s3
       if(ishuiwen(s3))
       num++;
    }
    return num;
